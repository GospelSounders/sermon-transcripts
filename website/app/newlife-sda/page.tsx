...
export default function NewlifeSDAPage() {
  ...
  useEffect(() => {
    // Load ministry data
    fetch('/sermon-transcripts/data/newlife-sda.json')
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div className="text-center">
                <div className="text-3xl font-bold text-green-600 mb-2">{ministryData.stats.total_sermons}</div>
                <div className="text-gray-600 text-sm">Total Sermons</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-green-600 mb-2">{ministryData.stats.languages.length}</div>
                <div className="text-gray-600 text-sm">Languages</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-green-600 mb-2">{ministryData.stats.topics.length}</div>
                ...
                <div key={sermon.id} className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button 
                        onClick={() => setSelectedTranscript({ ministry: 'newlife-sda', filename: sermon.filename })}
                      ...
                      <button 
                        onClick={async () => {
                          try {
                            const response = await fetch(`/sermon-transcripts/output/newlife-sda/${sermon.filename}`)
                            if (response.ok) {
